...
export class FullPage {
	constructor(element, options) {
		let config = {
			...
			classInit: 'fp-init',
			// Класс для врапера во время свертывания
			...
			// / / Класс оболочки буклетов
			bulletclass: 'fp-bullets',
			// Класс буллета
	...
	init() {
		if (this.options.idActiveSection > (this.section.length - 1)) return
		// Расставляем id
		this.site();
